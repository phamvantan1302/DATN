    ...
    <table class="table">
        ...
        <c:forEach items="${brand.content}" var="ls">
            <tr>
                ...
                <th>${ls.name}</th>
                <td>
                    <a class="btn btn-info" href="/admin/detail_brand/${ls.id}">Detail</a>
                    <a class="btn btn-danger" href="/admin/delete_brand/${ls.id}">Delete</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
        ...
        <ul class="pagination">
            <c:if test="${brand.totalPages > 1}">
                <c:forEach begin="0" end="${ brand.totalPages -1}" varStatus="loop">
                    <li class="page-item">
                        <a class="page-link" href="/admin/hienthi_brand?page=${loop.begin + loop.count - 1}">
